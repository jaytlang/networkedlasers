...
##SMSC Ethernet PHY: annotated

## MDIO/MDC: management interface to the phy, access registers within it? no touchy touchy
...
## CRSDV : Carrier Sense/Receive Data *Valid*
## Asserted by the PHY when the receive medium is not idle as defined by IEEE 802.3
set_property -dict { PACKAGE_PIN D9    IOSTANDARD LVCMOS33 } [get_ports { eth_crsdv }]; #IO_L6N_T0_VREF_16 Sch=eth_crsdv

## RXERR
## Something went bad, receive error
## Don't need it; our MAC deals with FCEs without this
# set_property -dict { PACKAGE_PIN C10   IOSTANDARD LVCMOS33 } [get_ports { eth_rxerr }]; #IO_L13N_T2_MRCC_16 Sch=eth_rxerr

## RXD
## For every clock period in which crsdv is asserted, two bits of recovered data from the phy are available
## Is == 0 when CRSDV is not asserted i.e. idle
## Out of band signaling when CRSDV is asserted and is != 0

# When CRSDV == 1, RXD initially will be 00 as decoding is taking place
## SSD is absorbed by RXD, so the first thing that pops on the line is the preamble (01)
## 10 means you got a false carrier
## Then SFD single octet, MAC-defined data, so on and so forth
set_property -dict { PACKAGE_PIN C11   IOSTANDARD LVCMOS33 } [get_ports { eth_rxd[0] }]; #IO_L13P_T2_MRCC_16 Sch=eth_rxd[0]
set_property -dict { PACKAGE_PIN D10   IOSTANDARD LVCMOS33 } [get_ports { eth_rxd[1] }]; #IO_L19N_T3_VREF_16 Sch=eth_rxd[1]

## TX
## Looks like you've gotta send the preamble too
set_property -dict { PACKAGE_PIN B9    IOSTANDARD LVCMOS33 } [get_ports { eth_txen }]; #IO_L11N_T1_SRCC_16 Sch=eth_txen
set_property -dict { PACKAGE_PIN A10   IOSTANDARD LVCMOS33 } [get_ports { eth_txd[0] }]; #IO_L14P_T2_SRCC_16 Sch=eth_txd[0]
set_property -dict { PACKAGE_PIN A8    IOSTANDARD LVCMOS33 } [get_ports { eth_txd[1] }]; #IO_L12N_T1_MRCC_16 Sch=eth_txd[1]

## REF_CLK
## 50 MHz clock passed in. Synchronous clock reference for receive, transmit, ctl interface
